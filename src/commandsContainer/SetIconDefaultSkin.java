...
public class SetIconDefaultSkin {
	public static void runTask(MessageReceivedEvent _e, int _default_skin, int _last_theme){
		if(_default_skin <= _last_theme){
			...
			RankingSystem.SQLUpdateIconDefaultSkin(_e.getGuild().getIdLong(), _e.getGuild().getName(), guild_settings.getIconID());
			
			Logger logger = LoggerFactory.getLogger(SetLevelDefaultSkin.class);
			logger.info("{} has set the default icon skin id to {} in guild {}", _e.getMember().getUser().getId(), guild_settings.getIconID(), _e.getGuild().getName());
