...
public class ShopExecution {
	...
	public static void displayPartOfShop(MessageReceivedEvent _e, String _type, String _description){
		...
		List<Skins> filteredContent = RankingSystem.SQLgetSkinshopContentAndType(_e.getGuild().getIdLong()).parallelStream().filter(e -> e.getSkinType().equals(_type)).collect(Collectors.toList());
	...
	public static void displayPartOfShopWeapons(MessageReceivedEvent _e, String _type) {
		List<Weapons> filteredContent = RankingSystemItems.SQLgetWholeWeaponShop(_e.getGuild().getIdLong()).parallelStream().filter(f -> f.getCategoryDescription().equalsIgnoreCase(_type) && f.getEnabled()).collect(Collectors.toList());
