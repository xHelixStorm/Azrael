...
public class Deny implements CommandPublic {
	...
	public boolean action(String[] args, MessageReceivedEvent e, BotConfigs botConfig) {
		...
		if(verification != null) {
			...
			if(category != null) {
				...
				if(ver_role != null) {
					if(args.length == 0) {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_DETAILS)).setDescription(STATIC.getTranslation(e.getMember(), Translation.DENY_HELP)).build()).queue();
					}
	...
	private static void kickUser(MessageReceivedEvent e, Member member, TextChannel textChannel, String reportReason) {
		e.getGuild().kick(member).reason(reportReason).queue(m -> {
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.DENY_DONE).replaceFirst("\\{\\}", member.getUser().getName()+"#"+member.getUser().getDiscriminator()).replace("{}", member.getUser().getId())).build()).queue();
