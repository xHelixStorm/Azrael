...
public class DoubleExperience implements CommandPublic {
	...
	public boolean action(String[] args, MessageReceivedEvent e, BotConfigs botConfig) {
		...
		if(guild_settings != null && guild_settings.getRankingState()) {
			...
			if(args.length == 0) {
				...
				e.getChannel().sendMessageEmbeds(message.setDescription(STATIC.getTranslation(e.getMember(), Translation.DOUBLE_EXPERIENCE_HELP).replace("{}", STATIC.getTranslation(e.getMember(), Translation.PARAM_STATE))
			...
			//display the current state of the double experience (enabled/disabled/auto)
				...
				e.getChannel().sendMessageEmbeds(message.setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.DOUBLE_EXPERIENCE_STATE).replace("{}", botConfig.getDoubleExperience())).build()).queue();
			}
			//change the state if either on, off or auto has been added as first parameter
				...
				if(!botConfig.getDoubleExperience().equalsIgnoreCase(getValue(e, args[0]))) {
					...
					e.getChannel().sendMessageEmbeds(message.setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.DOUBLE_EXPERIENCE_UPDATE).replace("{}", args[0].toUpperCase())).build()).queue();
					...
					if(args[0].equalsIgnoreCase(STATIC.getTranslation(e.getMember(), Translation.PARAM_ON))) {
						...
						if(bot_channel != null) {
							...
							if(textChannel != null && (e.getGuild().getSelfMember().hasPermission(textChannel, Permission.MESSAGE_ATTACH_FILES) || STATIC.setPermissions(e.getGuild(), textChannel, EnumSet.of(Permission.MESSAGE_ATTACH_FILES)))) {
								textChannel.sendFiles(FileUpload.fromData(doubleEvent, "doubleweekend.jpg")).queue();
					...
					e.getChannel().sendMessageEmbeds(message.setDescription(STATIC.getTranslation(e.getMember(), Translation.DOUBLE_EXPERIENCE_ERROR).replace("{}", args[0].toUpperCase())).build()).queue();
				}
			}
			else {
			...
			e.getChannel().sendMessageEmbeds(message.setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.LEVEL_SYSTEM_NOT_ENABLED)).build()).queue();
		}
		return true;
	}
	...
	private String getValue(MessageReceivedEvent e, String argument) {
