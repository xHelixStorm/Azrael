	...
	public boolean action(String[] args, MessageReceivedEvent e, BotConfigs botConfig) {
		...
		if(args.length > 0 && args[0].equalsIgnoreCase(STATIC.getTranslation(e.getMember(), Translation.PARAM_ENABLE))) {
			if(Azrael.SQLgetCommandExecutionReaction(e.getGuild().getIdLong()) == true) {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.ROLE_REACTION_ENABLED)).build()).queue();
			}
			else {
				if(Azrael.SQLUpdateReaction(e.getGuild().getIdLong(), true) > 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.ROLE_REACTION_ENABLE)).build()).queue();
			...
			if(Azrael.SQLgetCommandExecutionReaction(e.getGuild().getIdLong()) == false) {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.ROLE_REACTION_DISABLED)).build()).queue();
			}
			else {
				if(Azrael.SQLUpdateReaction(e.getGuild().getIdLong(), false) > 0) {
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.ROLE_REACTION_DISABLE)).build()).queue();
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_DETAILS)).setDescription(STATIC.getTranslation(e.getMember(), Translation.ROLE_REACTION_HELP)
