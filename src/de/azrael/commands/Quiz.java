...
public class Quiz implements CommandPublic {
	...
	public boolean action(String[] args, MessageReceivedEvent e, BotConfigs botConfig) {
		...
		if(Hashes.getTempCache("quiztime_gu"+e.getGuild().getId()) == null) {
			...
			if(args.length == 0) {
				e.getChannel().sendMessageEmbeds(message.setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_ABOUT)).setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_HELP)
				...
				if(args[0].equalsIgnoreCase(STATIC.getTranslation(e.getMember(), Translation.PARAM_REGISTER_REWARDS))) {
					e.getChannel().sendMessageEmbeds(message.setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_REWARDS_HELP)).build()).queue();
				...
				//help command to register questions
					...
					e.getChannel().sendMessageEmbeds(message.setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_QUESTIONS_HELP)).build()).queue();
				...
				//clear all questions and rewards
					...
					if(Azrael.SQLDeleteQuizData(e.getGuild().getIdLong()) >= 0) {
						...
						e.getChannel().sendMessageEmbeds(message.setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_CLEAR)).build()).queue();
					...
					if(Azrael.SQLgetQuizData(e.getGuild().getIdLong())) {
						...
						if(registeredQuiz != null && registeredQuiz.size() > 0 && registeredQuiz.values().parallelStream().filter(f -> !f.isUsed()).findAny().orElse(null) != null) {
							...
							if(quiz.getQuestion() == null || quiz.getQuestion().length() == 0) {
								e.getChannel().sendMessageEmbeds(message.setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_NO_Q_AND_A)).build()).queue();
							}
							else if(quiz.getReward() == null || quiz.getReward().length() == 0) {
								e.getChannel().sendMessageEmbeds(message.setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_NO_REWARDS)).build()).queue();
							}
							else {
								...
								if(textChannel != null && (e.getGuild().getSelfMember().hasPermission(textChannel, Permission.VIEW_CHANNEL, Permission.MESSAGE_SEND) || STATIC.setPermissions(e.getGuild(), textChannel, EnumSet.of(Permission.VIEW_CHANNEL, Permission.MESSAGE_SEND)))) {
									e.getChannel().sendMessageEmbeds(message.setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_RUN_HELP)).build()).queue();
									//write to cache to remind the bot that we're waiting for input
									Hashes.addTempCache("quizstarter_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), new Cache(180000));
									commandLevel.put(e.getGuild().getIdLong(), STATIC.getCommandLevel(e.getGuild(), Command.QUIZ));
								}
								else {
									e.getChannel().sendMessageEmbeds(message.setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_MISSING_PERMISSION)).build()).queue();
								}
							}
						}
						else {
							e.getChannel().sendMessageEmbeds(message.setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_NO_QA_REWARDS)).build()).queue();
						}
					}
					else {
						e.getChannel().sendMessageEmbeds(message.setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_NO_QA_REWARDS)).build()).queue();
			...
			e.getChannel().sendMessageEmbeds(denied.setDescription(STATIC.getTranslation(e.getMember(), Translation.QUIZ_ERR)).build()).queue();
		}
		return true;
	}
