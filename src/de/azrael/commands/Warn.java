					...
					if(member.getUser().getIdLong() != e.getMember().getUser().getIdLong()) {
						if(e.getGuild().getSelfMember().canInteract(member)) {
							...
							member.getUser().openPrivateChannel().queue(channel -> {
								channel.sendMessage(new EmbedBuilder().setColor(Color.ORANGE).setDescription(STATIC.getTranslation2(e.getGuild(), Translation.WARN_SENT).replaceFirst("\\{\\}", e.getGuild().getName()).replace("{}", reason)).build()).queue(success -> {
