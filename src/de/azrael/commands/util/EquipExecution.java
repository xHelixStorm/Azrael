...
public class EquipExecution {
		...
		if(foundGuilds != null) {
			if(foundGuilds.size() == 1) {
				...
				if(UserPrivs.comparePrivilege(member, commandLevel)) {
					...
					e.getChannel().sendMessageEmbeds(denied.setDescription(e.getAuthor().getAsMention() + STATIC.getTranslation3(e.getAuthor(), Translation.HIGHER_PRIVILEGES_ROLE) + UserPrivs.retrieveRequiredRoles(commandLevel, member)).build()).queue();
				}
			}
			else {
				...
				try {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SERVER_SELECT)+out.toString()).build()).queue();
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_NO_SERVER_FOUND)).build()).queue();
	...
	public static void selectAvailableGuilds(MessageReceivedEvent e, final String guilds, final int selection) {
		...
		if(selection >= 0 && selection < guild_ids.length) {
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SELECT_ERR)).build()).queue();
			var cache = Hashes.getTempCache("equip_us"+e.getAuthor().getId()).setExpiration(180000);
			Hashes.addTempCache("equip_us"+e.getAuthor().getId(), cache);
		}
	}
	...
	public static void equipmentItemScreen(MessageReceivedEvent e, final String guild_id, String action) {
		...
		e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_EQUIPMENT)
	...
	public static void removeWholeEquipment(MessageReceivedEvent e, final long guild_id) {
		...
		if(user_details.getWeapon1() != 0 || user_details.getWeapon2() != 0 || user_details.getWeapon3() != 0 || user_details.getSkill() != 0) {
			if(RankingSystemItems.SQLUnequipWholeEquipment(e.getAuthor().getIdLong(), guild_id) > 0) {
				...
				user_details.setWeapon3(0);
				user_details.setSkill(0);
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_UNEQUIP_ALL)).build()).queue();
			}
			else {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation3(e.getAuthor(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.GENERAL_ERROR)).build()).queue();
				logger.error("All weapons and skill slots couldn't be unequipped for user {} in guild {}", e.getAuthor().getId(), guild_id);
			}
		}
		else {
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_UNEQUIP_ALL_EMPTY)).build()).queue();
		}
		Azrael.SQLInsertCommandLog(e.getAuthor().getIdLong(), 0, Command.EQUIP.getColumn(), e.getMessage().getContentRaw());
	}
	...
	public static void slotSelection(MessageReceivedEvent e, final String guild_id, final int selection, String action) {
		if(selection > 0 && selection <= 4) {
			if(action.equals("set")) {
				switch(selection) {
					case 1, 2, 3 -> {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setTitle(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_TITLE_SLOT).replace("{}", ""+selection)).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SELECT_WEAPON)).build()).queue();
					}
					case 4 -> {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setTitle(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_TITLE_SKILL)).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SELECT_SKILL)).build()).queue();
					}
				}
				...
				if(itemID != 0) {
					if(RankingSystemItems.SQLUnequipWeapon(e.getAuthor().getIdLong(), guild, selection) > 0) {
						switch(selection) {
							case 1 -> {
								e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_WEP_UNEQUIPPED)).build()).queue();
								user_details.setWeapon1(0);
							}
							case 2 -> {
								e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_WEP_UNEQUIPPED)).build()).queue();
							...
							case 4 -> {
								e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SKILL_UNEQUIPPED)).build()).queue();
								user_details.setSkill(0);
							}
						}
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SLOT_EMPTY)).build()).queue();
					equipmentItemScreen(e, guild_id, "remove");
				}
			}
			Azrael.SQLInsertCommandLog(e.getAuthor().getIdLong(), 0, Command.EQUIP.getColumn(), e.getMessage().getContentRaw());
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SELECT_DIGIT)).build()).queue();
			var cache = Hashes.getTempCache("equip_us"+e.getAuthor().getId()).setExpiration(180000);
			Hashes.addTempCache("equip_us"+e.getAuthor().getId(), cache);
		}
	}
	...
	public static void searchInventory(MessageReceivedEvent e, final String guild_id, final int selection, final String item) {
		...
		if(selection >= 1 && selection <= 3) {
			...
			if(weapons == null) {
					...
					if(weapon.getItemId() != user_details.getWeapon1() && weapon.getItemId() != user_details.getWeapon2() && weapon.getItemId() != user_details.getWeapon3()) {
						...
						if(!weapon.getAbbreviation().equals((weapon1 != null ? weapon1.getWeaponAbbv() : "")) && !weapon.getAbbreviation().equals((weapon2 != null ? weapon2.getWeaponAbbv() : "")) && !weapon.getAbbreviation().equals((weapon3 != null ? weapon3.getWeaponAbbv() : ""))) {
							if(RankingSystemItems.SQLEquipWeapon(e.getAuthor().getIdLong(), guild, weapon.getItemId(), selection) > 0) {
								...
								e.getChannel().sendMessageEmbeds(embed.setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_EQUIPPED)).build()).queue();
							...
							e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_ALREADY_EQUIPPED)
						...
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_ALREADY_EQUIPPED_2)
								.replace("{}", STATIC.getTranslation3(e.getAuthor(), Translation.PARAM_RETURN))).build()).queue();
						var cache = Hashes.getTempCache("equip_us"+e.getAuthor().getId()).setExpiration(180000);
						Hashes.addTempCache("equip_us"+e.getAuthor().getId(), cache);
					}
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SELECT_WEAPON_2)+sb.toString()).build()).queue();
				Hashes.addTempCache("equip_us"+e.getAuthor().getId(), new Cache(180000, guild_id, "set-"+selection+"_"+items));
			}
			else {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SELECT_WEP_NOT_FOUND)
						.replace("{}", STATIC.getTranslation3(e.getAuthor(), Translation.PARAM_RETURN))).build()).queue();
				var cache = Hashes.getTempCache("equip_us"+e.getAuthor().getId()).setExpiration(180000);
				Hashes.addTempCache("equip_us"+e.getAuthor().getId(), cache);
			}
			...
			if(skills.size() == 1) {
				...
				if(skill.getItemId() != user_details.getSkill()) {
					if(RankingSystemItems.SQLEquipSkill(e.getAuthor().getIdLong(), guild, skill.getItemId()) > 0) {
						...
						Hashes.addRanking(guild, e.getAuthor().getIdLong(), user_details);
						EmbedBuilder embed = new EmbedBuilder().setColor(Color.BLUE);
						e.getChannel().sendMessageEmbeds(embed.setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SKILL_EQUIPPED)).build()).queue();
						equipmentItemScreen(e, guild_id, "set");
					}
					else {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation3(e.getAuthor(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.GENERAL_ERROR)).build()).queue();
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SKILL_ALREADY_EQUIPPED)
							.replace("{}", STATIC.getTranslation3(e.getAuthor(), Translation.PARAM_RETURN))).build()).queue();
					var cache = Hashes.getTempCache("equip_us"+e.getAuthor().getId()).setExpiration(180000);
					Hashes.addTempCache("equip_us"+e.getAuthor().getId(), cache);
				}
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SELECT_SKILL_2)+sb.toString()).build()).queue();
				Hashes.addTempCache("equip_us"+e.getAuthor().getId(), new Cache(180000, guild_id, "set-"+selection+"_"+items));
			}
			else {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation3(e.getAuthor(), Translation.EQUIP_SELECT_SKILL_NOT_FOUND)
						.replace("{}", STATIC.getTranslation3(e.getAuthor(), Translation.PARAM_RETURN))).build()).queue();
				var cache = Hashes.getTempCache("equip_us"+e.getAuthor().getId()).setExpiration(180000);
				Hashes.addTempCache("equip_us"+e.getAuthor().getId(), cache);
			}
