...
public class RegisterChannel {
	...
	public static void RegisterChannelHelper(MessageReceivedEvent e, Thumbnails thumbnails) {
		...
		if(channelTypes != null) {
			...
			if(strB.length() > 0) {
				e.getChannel().sendMessageEmbeds(messageBuild.setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_CHANNEL_HELP)).addField("", strB.toString(), true).addField("", strB2.toString(), true).build()).queue();
			}
			if(strB.length() == 0)
				e.getChannel().sendMessageEmbeds(messageBuild.setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_CHANNEL_NO_TYPES)).build()).queue();
		}
		else {
	...
	public static void RegisterChannelHelperURL(MessageReceivedEvent e) {
		...
		e.getChannel().sendMessageEmbeds(messageBuild.setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_CHANNEL_URL_HELP)
	...
	public static void RegisterChannelHelperTxt(MessageReceivedEvent e) {
		EmbedBuilder messageBuild = new EmbedBuilder().setColor(Color.BLUE).setThumbnail(BotConfiguration.SQLgetThumbnails(e.getGuild().getIdLong()).getSettings());
		e.getChannel().sendMessageEmbeds(messageBuild.setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_CHANNEL_TXT_HELP)
				.replaceFirst("\\{\\}", STATIC.getTranslation(e.getMember(), Translation.PARAM_ENABLE))
				.replace("{}", STATIC.getTranslation(e.getMember(), Translation.PARAM_DISABLE))).build()).queue();
	}
	
	public static boolean runCommand(MessageReceivedEvent e, String [] args, Thumbnails thumbnails, BotConfigs botConfig) {
		String channel;
		String channel_type;
				...
				if(args.length == 3 && matcher.find()) {
					...
					if(channel.length() > 0) {
						...
						if(textChannel != null) {
							...
							if(result > 0) {
								e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_CHANNEL_REGISTERED)).build()).queue();
	...
	public static boolean runCommandURL(MessageReceivedEvent e, String [] args, Thumbnails thumbnails) {
	...
	public static boolean runCommandTxt(MessageReceivedEvent e, String [] args, Thumbnails thumbnails) {
	...
	public static boolean runChannelsRegistration(MessageReceivedEvent e, Thumbnails thumbnails) {
		...
		if(UserPrivs.comparePrivilege(e.getMember(), commandLevel)) {
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ALL_CHANNELS)).build()).queue();
