...
public class SetLevelDefaultSkin {
	...
	public static void runTask(MessageReceivedEvent e, int defaultSkin, int lastTheme, ArrayList<UserLevel> skins) {
			...
			if(skin != null || defaultSkin == 0) {
				...
				if(RankingSystem.SQLUpdateLevelDefaultSkin(e.getGuild().getIdLong(), e.getGuild().getName(), skinId) > 0) {
					...
					if(RankingSystem.SQLUpdateUsersDefaultLevelSkin(guild_settings.getLevelID(), skinId, e.getGuild().getIdLong()) != -1) {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_LEVEL_UPDATE)+skinDescription).build()).queue();
