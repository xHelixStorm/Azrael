...
public class RegisterRole {
	...
	public static void RegisterRoleHelper(MessageReceivedEvent e, Thumbnails thumbnails, boolean adminRestriction) {
		...
		if(roles != null) {
			...
			if(strB.length() > 0) {
				e.getChannel().sendMessageEmbeds(messageBuild.setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ROLE_HELP)).addField("", strB.toString(), true).addField("", strB2.toString(), true).build()).queue();
			}
			else {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ROLE_NO_TYPES)).build()).queue();
			}
		}
		else {
	...
	public static void runCommandWithAdminFirst(MessageReceivedEvent e, String [] args, Thumbnails thumbnails) {
		...
		if(args.length == 3 && args[1].equalsIgnoreCase("adm") && args[2].matches("[0-9]*")) {
			...
			if(role != null) {
				if(DiscordRoles.SQLInsertRole(e.getGuild().getIdLong(), role.getIdLong(), STATIC.getLevel(category_abv), role.getName(), category_abv, false) > 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ROLE_ADM_ADDED)).build()).queue();
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ROLE_INVALID_PARAM)).build()).queue();
		}
	}

	public static boolean runCommand(MessageReceivedEvent e, String [] args, Thumbnails thumbnails) {
		String category_abv = null;
		...
		if(UserPrivs.comparePrivilege(e.getMember(), commandLevel)) {
						...
						if(role != null) {
							...
							if(category_abv.equals("rea")) {
								...
								if(reactionRolesSize == 10) {
									e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ROLE_REA_ERR)).build()).queue();
									return true;
								}
							}
							if(DiscordRoles.SQLInsertRole(e.getGuild().getIdLong(), role.getIdLong(), level, role.getName(), category_abv, persistant) > 0) {
								...
								e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ROLE_ADDED)).build()).queue();
								Hashes.removeDiscordRoles(e.getGuild().getIdLong());
								if(category_abv.equals("rea")) {
									...
									if(!Azrael.SQLisGiveawayAvailable(e.getGuild().getIdLong())) {
										e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.ORANGE).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ROLE_KEY)).build()).queue();
									}
								}
								DiscordRoles.SQLgetRoles(e.getGuild().getIdLong());
							}
			...
			e.getChannel().sendMessageEmbeds(denied.setDescription(e.getMember().getAsMention() + STATIC.getTranslation(e.getMember(), Translation.HIGHER_PRIVILEGES_ROLE) + UserPrivs.retrieveRequiredRoles(commandLevel, e.getMember())).build()).queue();
		}
		return false;
	}
	
	public static void assignVerifiedRoleToMembers(MessageReceivedEvent e) {
		if(e.getMessage().getContentRaw().equalsIgnoreCase(STATIC.getTranslation(e.getMember(), Translation.PARAM_YES))) {
			if(e.getGuild().getSelfMember().hasPermission(Permission.MANAGE_ROLES)) {
				...
				if(ver_role != null) {
					...
					if(role != null) {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ROLE_ASSIGN_START)).build()).queue();
						...
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ROLE_DONE)).build()).queue();
					}
					else {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation(e.getMember(), Translation.GENERAL_ERROR)).build()).queue();
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_PERMISSIONS)).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_ROLE_ERR).replace("{}", Permission.MANAGE_ROLES.getName())).build()).queue();
