...
public class SetMap {
	...
	public static void runHelp(MessageReceivedEvent e) {
		e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_DETAILS)).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_MAP_HELP)
	...
	public static void runTask(MessageReceivedEvent e, String [] args) {
		if(args[1].equalsIgnoreCase(STATIC.getTranslation(e.getMember(), Translation.PARAM_CLEAR))) {
			if(args.length == 2) {
				if(Competitive.SQLClearMaps(e.getGuild().getIdLong()) >= 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_MAP_CLEARED)).build()).queue();
			...
			if(args.length > 2) {
				...
				if(result > 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_MAP_REMOVED)).build()).queue();
					logger.info("User {} has removed the competitive map {} in guild {}", e.getMember().getUser().getId(), removeMap, e.getGuild().getId());
				}
				else if(result == 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_MAP_REMOVE_ERR)).build()).queue();
				}
				else {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation(e.getMember(), Translation.GENERAL_ERROR)).build()).queue();
		...
		if(Competitive.SQLInsertMap(e.getGuild().getIdLong(), insertMap, url) > 0) {
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_MAP_SUCCESS)).build()).queue();
