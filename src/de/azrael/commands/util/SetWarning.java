...
public class SetWarning {
	...
	public static void runHelp(MessageReceivedEvent e) {
		...
		e.getChannel().sendMessageEmbeds(messageBuild.setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_WARNING_HELP)).build()).queue();
	}
	
	public static void runTask(MessageReceivedEvent e, String message) {
		int warningValue = 0;
		...
		if(warningValue > 0 && warningValue <= 30) {
			...
			if(editedRows > 0) {
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_WARNING_1).replace("{}", ""+warningValue)).build()).queue();
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_WARNING_NOT_VALID)).build()).queue();
		}
	}
	
	public static void performUpdate(MessageReceivedEvent e, String message, Cache cache, String key) {
		if(message.matches("[0-9]{1,}")) {
			if(cache.getExpiration() - System.currentTimeMillis() > 0) {
				...
				if(value <= maxWarning) {
					if(Azrael.SQLUpdateMuteTimeOfWarning(e.getGuild().getIdLong(), value, (Long.parseLong(message)*60*1000)) > 0) {
						if(value < maxWarning) {
							e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_WARNING_2).replaceFirst("\\{\\}", ""+value).replace("{}", ""+(value+1))).build()).queue();
							...
							e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_WARNINIG_ADDED)).build()).queue();
							logger.info("The timer of warning {} has been updated to {} in guild {}", value, message, e.getGuild().getId());
