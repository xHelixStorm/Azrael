...
public class RegisterRankingRole {
	...
	public static void RegisterRankingRoleHelper(MessageReceivedEvent e, Thumbnails thumbnails) {
		EmbedBuilder messageBuild = new EmbedBuilder().setColor(Color.BLUE).setThumbnail(thumbnails.getSettings());
		e.getChannel().sendMessageEmbeds(messageBuild.setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_RANK_ROLE_HELP)).build()).queue();
	}
		...
		if(UserPrivs.comparePrivilege(e.getMember(), commandLevel)) {
			if(args.length == 3 && args[2].matches("[0-9]*")) {
				...
				if(args[1].matches("[0-9]*")) {
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_RANK_ROLE_NO_LEVEL)).build()).queue();
					return true;
				}
			}
			else {
			...
			if(level_requirement < 1 || level_requirement > 10000) {
				...
				if(RankingSystem.SQLInsertRole(role.getIdLong(), role.getName(), level_requirement, guild_id) != -1) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_RANK_ROLE_ADDED).replaceFirst("\\{\\}", role.getName()).replace("{}", ""+level_requirement)).build()).queue();
					...
					if(RankingSystem.SQLgetRoles(guild_id).size() > 0) {
						if(RankingSystem.SQLgetLevels(guild_id).size() == 0) {
							e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.ORANGE).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_WARNING)).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_RANK_ROLE_WARN)).build()).queue();
