...
public class RegisterCategory {
	...
	public static void runHelp(MessageReceivedEvent e, Thumbnails thumbnails) {
		...
		if(categoryTypes != null) {
			...
			if(strB.length() > 0) {
				e.getChannel().sendMessageEmbeds(messageBuild.setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_CATEGORY_HELP)).addField("", strB.toString(), true).addField("", strB2.toString(), true).build()).queue();
			}
			else {
				e.getChannel().sendMessageEmbeds(messageBuild.setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_CATEGORY_NO_TYPES)).build()).queue();
			}
		}
		else {
					...
					if(category.length() > 0) {
						...
						if(guildCategory != null) {
							...
							if(result > 0) {
								...
								e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_CATEGORY_REGISTERED)).build()).queue();
							}
							else {
								e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation(e.getMember(), Translation.GENERAL_ERROR)).build()).queue();
