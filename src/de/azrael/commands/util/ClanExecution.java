...
public class ClanExecution {
	...
	public static void search(MessageReceivedEvent e, String [] args, Cache cache) {
		...
		if(clans != null && clans.size() > 0) {
			...
			if(filteredClans.size() > 0) {
				...
				if(filteredClans.size() > 10) {
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setTitle(STATIC.getTranslation(e.getMember(), Translation.CLAN_TITLE)).setDescription(out.toString()).build()).queue(m -> {
						if(e.getGuild().getSelfMember().hasPermission(e.getGuildChannel(), Permission.MESSAGE_ADD_REACTION) || STATIC.setPermissions(e.getGuild(), e.getChannel().asTextChannel(), EnumSet.of(Permission.MESSAGE_ADD_REACTION))) {
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setTitle(STATIC.getTranslation(e.getMember(), Translation.CLAN_TITLE)).setDescription(out.toString()).build()).queue();
			}
			else {
				//clan hasn't been found
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_SEARCH_ERR)).build()).queue();
			}
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_NO_CLANS)).build()).queue();
		}
		else {
	...
	public static void apply(MessageReceivedEvent e, String [] args, Cache cache) {
		String name = "";
		for(int i = 1; i < args.length; i++) {
			name = name+args[i]+" ";
		}
		if(name.length() == 0) {
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_APPLY_ERR)).build()).queue();
			Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
			return;
		}
		...
		if(clan_id > 0) {
			...
			if(management != null && management.size() > 0) {
				...
				if(username != null) {
					...
					if(result != null && (result.getClanID() == 0 || result.getType() != 1 || (result.getType() == 1 && result.isDone()))) {
						...
						if(Competitive.SQLInsertClanReservation(e.getGuild().getIdLong(), e.getMember().getUser().getIdLong(), clan_id, 1, e.getChannel().getIdLong()) > 0) {
							e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_APPLY_SUBMITTED)).build()).queue();
						...
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_APPLY_ALREADY_SUBMITTED)).build()).queue();
						Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
					}
					else {
						//error
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_APPLY_NO_MANAGER)).build()).queue();
			...
			//clan doesn't exist
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_APPLY_NOT_FOUND)).build()).queue();
			Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
		}
		else {
			//error
	...
	public static void create(MessageReceivedEvent e, String [] args, Cache cache) {
		String name = "";
		for(int i = 1; i < args.length; i++) {
			name = name + args[i]+" ";
		}
			...
			if(name.length() <= 30) {
				...
				if(clan_id == 0) {
					if(Competitive.SQLCreateClan(e.getGuild().getIdLong(), e.getMember().getUser().getIdLong(), name) > 0) {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_CREATE_SUCCESS).replace("{}", name)).build()).queue();
						Hashes.clearTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId());
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_CREATE_ERR_2)).build()).queue();
					Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
				}
				else {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation(e.getMember(), Translation.GENERAL_ERROR)).build()).queue();
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_CREATE_ERR_3)).build()).queue();
				Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
			}
		}
		else {
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_CREATE_ERR)).build()).queue();
			Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
		}
		Azrael.SQLInsertCommandLog(e.getMember().getUser().getIdLong(), e.getGuild().getIdLong(), Command.CLAN.getColumn(), e.getMessage().getContentRaw());
	}
	
	public static void members(MessageReceivedEvent e, Cache cache) {
		...
		if(members != null && members.size() > 0) {
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setTitle(STATIC.getTranslation(e.getMember(), Translation.CLAN_MEMBERS_TITLE))
	...
	public static void leave(MessageReceivedEvent e, String [] args, Cache cache) {
		final int clan_id = Integer.parseInt(cache.getAdditionalInfo());
		...
		if(submittedName.length() == 0) {
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_LEAVE_ERR)).build()).queue();
			Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
			return;
		}
		...
		if(clanName != null) {
			if(clanName.equals(submittedName)) {
				if(Competitive.SQLRemoveClanMember(e.getGuild().getIdLong(), e.getMember().getUser().getIdLong(), clan_id) > 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_LEAVE_LEFT)).build()).queue();
					Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
					...
					if(management != null && management.size() > 0) {
						...
						if(username != null) {
							management.parallelStream().forEach(user_id -> {
								e.getGuild().getMemberById(user_id).getUser().openPrivateChannel().queue(channel -> {
									channel.sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_LEAVE_DM).replace("{}", username)).build()).queue();
								});
							});
						}
						else
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_LEAVE_ERR_2)).build()).queue();
				Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
			}
		}
		else {
	...
	public static void kick(MessageReceivedEvent e, String [] args, int memberLevel, Cache cache) {
		final int clan_id = Integer.parseInt(cache.getAdditionalInfo());
		...
		if(member != null) {
			if(Competitive.SQLRemoveClanMember(e.getGuild().getIdLong(), member.getUserID(), clan_id) > 0) {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_KICKED).replace("{}", member.getUsername())).build()).queue();
				Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
				...
				if(guildMember != null) {
					guildMember.getUser().openPrivateChannel().queue(channel -> {
						channel.sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_KICK_DM).replace("{}", member.getClanName())).build()).queue(m -> {
							//notification received
						}, err -> {
							e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_DM_LOCKED)).build()).queue();
						});
					});
				}
				else {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_NOT_IN_GUILD)).build()).queue();
				}
			}
			else {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation(e.getMember(), Translation.GENERAL_ERROR)).build()).queue();
	...
	public static void invite(MessageReceivedEvent e, String [] args, Cache cache) {
		final int clan_id = Integer.parseInt(cache.getAdditionalInfo());
		...
		if(member != null) {
			...
			if(guildMember != null) {
				...
				if(clanName != null) {
					...
					if(result != null && (result.getClanID() == 0 || result.getType() != 1 || (result.getType() == 1 && result.isDone()))) {
						...
						if(Competitive.SQLInsertClanReservation(e.getGuild().getIdLong(), member.getUserID(), clan_id, 2, e.getChannel().getIdLong()) > 0) {
							...
							guildMember.getUser().openPrivateChannel().queue(channel -> {
								channel.sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setFooter(e.getGuild().getId()+"-"+member.getUserID()+"-"+clan_id).setDescription(STATIC.getTranslation2(e.getGuild(), Translation.CLAN_INVITE_DM).replace("{}", clanName)).build()).queue(m -> {
									m.addReaction(Emoji.fromUnicode(EmojiManager.getForAlias(":white_check_mark:").getUnicode())).queue();
									m.addReaction(Emoji.fromUnicode(EmojiManager.getForAlias(":x:").getUnicode())).queue();
									e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_INVITE_SENT)).build()).queue();
								}, err -> {
									e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_INVITE_DM_ERR)).build()).queue();
								});
							});
							Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
						}
						...
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_INVITE_ALREADY_SENT)).build()).queue();
						Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
					}
					else {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation(e.getMember(), Translation.GENERAL_ERROR)).build()).queue();
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_INVITE_ERR)).build()).queue();
				Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
			}
		}
		Azrael.SQLInsertCommandLog(e.getMember().getUser().getIdLong(), e.getGuild().getIdLong(), Command.CLAN.getColumn(), e.getMessage().getContentRaw());
	...
	public static void promote(MessageReceivedEvent e, String [] args, Cache cache) {
		final int clan_id = Integer.parseInt(cache.getAdditionalInfo());
		...
		if(member != null) {
			...
			if(memberLevel > 0) {
				...
				if(memberLevel == 1) {
					...
					if(result > 0) {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_PROMOTE_STAFF).replace("{}", member.getUsername())).build()).queue();
						Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
					...
					if(result > 0) {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_PROMOTE_MEMBER).replace("{}", member.getUsername())).build()).queue();
						Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
	...
	public static void icon(MessageReceivedEvent e, String [] args, Cache cache) {
		if(args.length == 2) {
			final int clan_id = Integer.parseInt(cache.getAdditionalInfo());
			...
			if(url.startsWith("http") && (url.endsWith(".png") || url.endsWith(".jpg") || url.endsWith(".gif"))) {
				if(Competitive.SQLUpdateClanMark(e.getGuild().getIdLong(), clan_id, url) > 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_ICON_UPDATED)).build()).queue();
					Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
				}
				else {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation(e.getMember(), Translation.GENERAL_ERROR)).build()).queue();
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_ICON_ERR)).build()).queue();
				Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
			}
		}
		else {
	...
	public static void delegate(MessageReceivedEvent e, String [] args, Cache cache) {
		final int clan_id = Integer.parseInt(cache.getAdditionalInfo());
		final var member = validateUser(e, args, cache, clan_id, 0, true, false, false);
		if(member != null) {
			if(Competitive.SQLDelegateOwnership(e.getGuild().getIdLong(), member.getUserID(), e.getMember().getUser().getIdLong(), clan_id) > 0) {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_DELEGATE_SUCCESS).replace("{}", member.getUsername())).build()).queue();
				Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
	...
	public static void disband(MessageReceivedEvent e, Cache cache) {
		final int clan_id = Integer.parseInt(cache.getAdditionalInfo());
		final var members = Competitive.SQLgetClanMembers(e.getGuild().getIdLong(), clan_id);
		if(members != null && members.size() > 0) {
			if(members.size() == 1) {
				if(Competitive.SQLDisbandClan(e.getGuild().getIdLong(), e.getMember().getUser().getIdLong(), clan_id) > 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_DISBAND_SUCCESS)).build()).queue();
					Hashes.clearTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId());
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_DISBAND_ERR)).build()).queue();
				Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
			}
		}
		else {
	...
	private static ClanMember validateUser(MessageReceivedEvent e, String [] args, Cache cache, int clan_id, int memberLevel, boolean validateClan, boolean validateUser, boolean validateNotInClan) {
		if(args.length > 1) {
			...
			if(member == null) {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_USER_NOT_FOUND)).build()).queue();
				Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
				return member;
			}
			else {
				if(e.getMember().getUser().getIdLong() == member.getUserID()) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_NOT_ON_YOURSELF)).build()).queue();
					Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
					return null;
				}
				if(validateClan && member.getClanID() != clan_id) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_USER_NOT_IN_SAME_CLAN)).build()).queue();
					Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
					return null;
				}
				if(validateUser) {
					if(member.getMemberLevel() == memberLevel || memberLevel < member.getMemberLevel()) {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_NOT_ALLOWED)).build()).queue();
						Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
						return null;
					}
				}
				if(validateNotInClan && member.getClanID() != 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_ALREADY_IN_CLAN)).build()).queue();
					Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
					return null;
				}
				return member;
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.CLAN_INVALID_NAME)).build()).queue();
			Hashes.addTempCache("clan_gu"+e.getGuild().getId()+"ch"+e.getChannel().getId()+"us"+e.getMember().getUser().getId(), cache.setExpiration(180000));
			return null;
		}
	}
