		...
		if(!type.equals("wep") && !type.equals("ski")) {
			...
			if(user_details.getCurrency() >= skin.getPrice()) {
				...
				if(editedRows > 0) {
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SHOP_PURCHASED).replace("{}", skin.getShopDescription())).build()).queue();
				...
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.SHOP_NOT_ENOUGH_CUR)).build()).queue();
				returnSkinMenu(e, guild_settings, type);
			}
		}
			...
			if(user_details.getCurrency() >= weapon.getPrice()) {
				...
				if(editedRows > 0) {
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SHOP_PURCHASED).replace("{}", weapon.getDescription()+(weapon.getStatDescription() != null ? " "+weapon.getStatDescription() : ""))).build()).queue();
			...
			if(user_details.getCurrency() >= skill.getPrice()) {
				...
				if(editedRows > 0) {
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SHOP_PURCHASED).replace("{}", skill.getDescription())).build()).queue();
	...
	public static void sell(MessageReceivedEvent e, final String type, final String item_number, Guilds guild_settings) {
		...
		if(!type.equals("wep") && !type.equals("ski")) {
			...
			if(RankingSystem.SQLUpdateCurrencyAndRemoveInventory(e.getMember().getUser().getIdLong(), e.getGuild().getIdLong(), newCurrency, skin.getItemID(), user_details.getLastUpdate()) > 0) {
				...
				Hashes.addRanking(e.getGuild().getIdLong(), e.getMember().getUser().getIdLong(), user_details);
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SHOP_SOLD)).build()).queue();
				returnSkinMenu(e, guild_settings, type);
			}
			else {
				returnSkinMenu(e, guild_settings, type);
