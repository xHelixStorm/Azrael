...
public class MeowExecution {
	public static void Execute(MessageReceivedEvent e, String [] _variable, String _path, long channel_id) throws IOException{
		EmbedBuilder messageBuild = new EmbedBuilder().setColor(Color.PINK).setThumbnail(BotConfiguration.SQLgetThumbnails(e.getGuild().getIdLong()).getMeow());
		...
		if(cache == null || cache.getExpiration() - System.currentTimeMillis() <= 0) {
			if(variable.length == 0) {
				e.getChannel().sendMessageEmbeds(messageBuild.setDescription(MeowUsage.getMeowInfos(e)+"\n\n[Pinterest](https://www.pinterest.de/caicaijade08/pusheen-pugly/)").build()).queue();
			}
			else {
				...
				if(variable[0].equalsIgnoreCase("meow")) {
					...
					e.getChannel().sendFiles(FileUpload.fromData(file, "meow.png")).queue();
				}
