...
public class SetRankingSystem {
	...
	public static void runTask(MessageReceivedEvent e, String input) {
		...
		if(input.equalsIgnoreCase(STATIC.getTranslation(e.getMember(), Translation.PARAM_ENABLE))) {
			...
			if(levels.size() == 0) {
				e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.REGISTER_RANK_ROLE_WARN)).build()).queue();
		...
		if(wrongInput == false) {
			...
			e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(message).build()).queue();
		}
	}
}
