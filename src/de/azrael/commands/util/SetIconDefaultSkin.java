...
public class SetIconDefaultSkin {
	...
	public static void runTask(MessageReceivedEvent e, int defaultSkin, int lastTheme, ArrayList<UserIcon> skins) {
			...
			if(skin != null || defaultSkin == 0) {
				...
				if(RankingSystem.SQLUpdateIconDefaultSkin(e.getGuild().getIdLong(), e.getGuild().getName(), skinId) > 0) {
					...
					if(RankingSystem.SQLUpdateUsersDefaultIconSkin(guild_settings.getIconID(), skinId, e.getGuild().getIdLong()) != -1) {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_ICON_UPDATE)+skinDescription).build()).queue();
