...
public class SetRankDefaultSkin {
	...
	public static void runTask(MessageReceivedEvent e, int defaultSkin, int lastTheme, ArrayList<UserRank> skins) {
			...
			if(skin != null || defaultSkin == 0) {
				...
				if(RankingSystem.SQLUpdateRankDefaultSkin(e.getGuild().getIdLong(), e.getGuild().getName(), skinId) > 0) {
					...
					if(RankingSystem.SQLUpdateUsersDefaultRankSkin(guild_settings.getRankID(), skinId, e.getGuild().getIdLong()) != -1) {
						e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_RANK_UPDATE)+skinDescription).build()).queue();
