...
public class SetCompServer {
	...
	public static void runHelp(MessageReceivedEvent e) {
		e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_DETAILS)).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_COMP_SERVER_HELP)
	...
	public static void runTask(MessageReceivedEvent e, String [] args) {
		if(args.length == 3) {
			if(args[1].equalsIgnoreCase(STATIC.getTranslation(e.getMember(), Translation.PARAM_ADD))) {
				...
				if(result > 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_COMP_SERVER_ADD).replace("{}", server)).build()).queue();
					logger.info("User {} has added the competitive server {} in guild {}", e.getMember().getUser().getId(), server, e.getGuild().getId());
				}
				else if(result == 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_COMP_SERVER_READD).replace("{}", server)).build()).queue();
				}
				else {
				...
				if(result > 0) {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_COMP_SERVER_REMOVE).replace("{}", server)).build()).queue();
					...
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.SET_COMP_SERVER_NOT_EXISTS).replace("{}", server)).build()).queue();
				}
				else {
					e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_ERROR)).setDescription(STATIC.getTranslation(e.getMember(), Translation.GENERAL_ERROR)).build()).queue();
