				...
				if(room.getChannelID() == e.getChannel().getIdLong() && room.getStatus() == 1 && (room.getLastJoined().getTime()+1800000)-System.currentTimeMillis() > 0) {
					...
					if(fromMember != null) {
						if(fromMember.isMaster()) {
							if(args.length == 0) {
								e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_DETAILS)).setDescription(STATIC.getTranslation(e.getMember(), Translation.RESTRICT_HELP)
									...
									if(memberLimit <= maxMemberLimit && memberLimit >= 2) {
										if(memberLimit % 2 == 0) {
											if(room.getMembers() <= memberLimit) {
												...
												e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.RESTRICT_ERR2)).build()).queue();
											}
										}
											...
											e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setDescription(STATIC.getTranslation(e.getMember(), Translation.RESTRICT_ERR)
													.replace("{}", STATIC.getTranslation(e.getMember(), Translation.PARAM_MAX))).build()).queue();
										}
									}
								}
								...
								if(allowed) {
									...
									if(Competitive.SQLUpdateRoomMemberLimit(e.getGuild().getIdLong(), room.getRoomID(), memberLimit) >= 0) {
										e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.BLUE).setDescription(STATIC.getTranslation(e.getMember(), Translation.RESTRICT_SUCCESS).replaceFirst("\\{\\}", ""+room.getRoomID()).replace("{}", ""+memberLimit)).build()).queue();
