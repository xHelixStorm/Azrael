...
public class UserPrivs {
	...
	private static boolean evaluateRequiredRole(int requiredLevel, MessageReceivedEvent e) {
					...
					if(currentRole != null) {
						...
						if(currentRole.getLevel() >= requiredLevel) {
							...
							if(rankingRole != null) {
								...
								e.getChannel().sendMessageEmbeds(new EmbedBuilder().setColor(Color.RED).setThumbnail(BotConfiguration.SQLgetThumbnails(e.getGuild().getIdLong()).getDenied()).setTitle(STATIC.getTranslation(e.getMember(), Translation.EMBED_TITLE_DENIED))
	...
	public static void throwNotEnoughPrivilegeError(MessageReceivedEvent e, int requiredLevel) {
