package de.azrael.sql;

import de.azrael.util.STATIC;

class DiscordRolesStatements {
	final static String SQLInsertRole = STATIC.decrypt("");
	final static String SQLInsertRoles = STATIC.decrypt("");
	final static String SQLUpdateAllRoles = STATIC.decrypt("Fx4SAO5otvhxOoRg9Tn8t7MQwxjL1olpMQWMXDM2rxK+kcRkwe1Cd0g4tSlnsdRL0PgPTg/SZ2fOVVexJvgez9ZWIjjkOUE++e6vP5Ist+SCJg1PT6IgCXfXespXSED/");
	final static String SQLUpdateLevel = STATIC.decrypt("Fx4SAO5otvhxOoRg9Tn8t4rxNLCQ0cbL4+ZjVDzgsTbgJtM1Ojq73oEMCytn3YOuYAF7rKteM/PYX/x/K7pAcw==");
	final static String SQLgetRoles = STATIC.decrypt("qVnLeov84sW1cHMhxUeRhwh8GnBzNnkWmtlGZy0RpfMCuZtSBpv8l10BXSsavj4P");
	final static String SQLgetReactionRoles = STATIC.decrypt("qVnLeov84sW1cHMhxUeRhwh8GnBzNnkWmtlGZy0RpfMUwGpwagwOn1KjSEAqitE4Nm/EpJij++kBCdUWdS1ihvExfnYAbvSzxtJGBhPT1nc=");
	final static String SQLUpdateRoleName = STATIC.decrypt("Fx4SAO5otvhxOoRg9Tn8t/kAzTj5ONpEtnV3ahqi8FfdYF4nG8li2A5at9iQodEQ/32X/IcoYPAaokDEuNFLbA==");
	final static String SQLgetCategories = STATIC.decrypt("3DUFKdo+5fK8jrz0VOsUfG23c2HdRhUjX/lTqqaEpaE=");
	final static String SQLDeleteRole = STATIC.decrypt("u5nvCcP2ijL2bIwgwzOg2HKX3IaS/MUzXZ+1mail1DpwQPEZBNd1BYCvmMOn9+ul0s306qDaJwzO025LVuUgFw==");
	final static String SQLUpdateRole = STATIC.decrypt("Fx4SAO5otvhxOoRg9Tn8t7MQwxjL1olpMQWMXDM2rxK+kcRkwe1Cd0g4tSlnsdRL0PgPTg/SZ2fOVVexJvgezyNxtsb+QOyenL+gUl+os1rP6PswXp7xmAR2G1J/JIdxBnJ+hkt/X4MEX2fa1H+tNg==");
	final static String SQLInsertReaction = STATIC.decrypt("4YprdPAF54DXsA9NAmFQ6VVlTXXx+p210B9wPhbi6Xg8/JE+TcM+nbP3zB3jT1+8ZI9u1nKoDpFL6qEclQVC/Dt8Nh226qZ7tcxR0NFnS38=");
	final static String SQLgetReactionRole = STATIC.decrypt("oqKjhmds/dioRA35XwKhctC3dhNQ0hRTTh8rt2KvP346EZO6vI6EbvStfLAB2xQHxrZvqGdCIchtrImBEbfxzPYYX5VoKQgPDWZIdCKoCbo=");
	final static String SQLDeleteReactions = STATIC.decrypt("XKwR4GJFw+nq1gXil4tt7ebIGLhgkULDlgFQTw6urWCwYQQ38g5bx8gmnfI/z63l");
	final static String SQLInsertReassignRoles = STATIC.decrypt("TpZ2CLnL8aPerayv2o7fTW6yUMUrxhMvslxFEULcpBqauuoYSHHJDg5vsWt8z604NpLo7ugXPtLrNynCf3NFD8wmj/rcKyw9MJWv8aHAvoM=");
	final static String SQLInsertReassignRoles2 = STATIC.decrypt("h+fKtWE0ghP/2yfMkhbe6RSB5dYvKHB39uzFufo+m8W6AU/B0vvusdUM+NPlQwZeC4QNJRMyP2COfxXwTyXmhWXoXazXZNaa5rKZS8hY+Js7fDYdtuqme7XMUdDRZ0t/");
	final static String SQLgetReassignRoles = STATIC.decrypt("IhjFBuEqt3bw/b2cckkdyKK2O4XA6PrDaM1dNnVwKh+7Ge9bFmqAgThYdZvIW2k1FgK4xNiWkmBRZ5VJU9YxtmABe6yrXjPz2F/8fyu6QHM=");
	final static String SQLgetReassignRoles2 = STATIC.decrypt("TpZ2CLnL8aPerayv2o7fTW6yUMUrxhMvslxFEULcpBqauuoYSHHJDg5vsWt8z604NpLo7ugXPtLrNynCf3NFD8wmj/rcKyw9MJWv8aHAvoM=");
}
