	...
	public void action(String[] args, MessageReceivedEvent e) {
		if(IniFileReader.getProfileCommand().equals("true")){
			...
			executor.execute(() -> {
				...
				int rank = 0;
				String fileName = IniFileReader.getTempDirectory()+"CommandDelay/"+user_id+"_profile.azr";
				...
				if(ranking_state == true){				
					if(!file.exists()){
						...
						new Thread(new ProfileDelay(fileName)).start();
						...
						RankingMethods.getProfile(e, convertedExperience, level, currentExperience, rankUpExperience, experience, currency, rank, profile_skin, icon_skin, bar_color, additional_text, color_r, color_g, color_b, rankx, ranky, rank_width, rank_height);
					}
					else{
