		...
		if(GuildIni.getCacheLog(e.getGuild().getIdLong())) {
			...
			if(cache == null) {
				...
				if(cache == null) {
					...
					else if(GuildIni.getSelfDeletedMessage(e.getGuild().getIdLong()) && removed_message != null && !UserPrivs.isUserBot(e.getGuild().getMemberById(removed_message.getUserID()).getUser(), e.getGuild().getIdLong())) {
